...
- **价格参考**: 按需实例,通过 EC2 Capacity Blocks 预留
- **性能提升**: 比Trn1快4倍,内存带宽4倍,内存容量3倍
- **计算能力**: 20.8 petaflops FP8

#### trn2u.48xlarge (UltraServer)
- **EC2 实例名称**: trn2u.48xlarge
- **加速器数量**: 64 (4个实例通过NeuronLink互联)
- **加速器内存**: 6TB HBM (总)
- **vCPU**: 192
- **系统内存**: 2TB
- **网络带宽**: 12.8 Tbps (EFA)
- **发布时间**: 2024年
- **Workload类型**: 训练 + 推理
- **适用场景**: 超大规模模型训练、最高性能推理
- **计算能力**: 83.2 petaflops FP8
- **内存带宽**: 185 TB/s (总)
