...
def save_meta(instances: List[Dict]) -> None:
    """写出 data/meta.js，供 index.html 读取更新日期与实例总数。

    index.html 是手工维护的，这两个值以前写死在 HTML 里，每次更新数据都会漏改，
    导致页面显示的日期和总数与 data/ 不一致。改为从这里读，就不会再漂移。
    """
    output_file = DATA_DIR / 'meta.js'
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write('// Auto-generated by scripts/convert_data.py - do not edit by hand\n')
        f.write('const gpuDataMeta = ')
        f.write(json.dumps(meta, indent=2, ensure_ascii=False))
        f.write(';\n')

    print(f"   元数据: meta.js ({meta['totalInstances']} 实例 / {meta['totalFamilies']} 家族)")
...
def process_global_data(use_official: bool = True) -> bool:
    ...
    # 5.1 写出元数据（index.html 读它显示日期与总数）
    save_meta(converted)
