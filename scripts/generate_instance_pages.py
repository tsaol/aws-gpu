...
<body>
    <div class="container">
        ...
        <div class="info-bar">
            <div class="info-item">
                <span class="info-label">GPU/Accelerator</span>
                <span class="info-value">{description}</span>
            </div>
            <div class="info-item">
                <span class="info-label">Instance Count</span>
                <span class="info-value" id="instanceCount">{instance_count} Variants</span>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
                ...
                <span class="info-value">{use_case}</span>
            </div>
            <div class="info-item">
                <span class="info-label">Current Region</span>
                <span class="info-value" id="currentRegionLabel">US East (N. Virginia)</span>
            </div>
        </div>
        ...
        <footer>
            ...
            <p style="margin-top: 8px; font-size: 0.9em; opacity: 0.8;">
                Data updated: 2025-11-30. Prices subject to change.
            </p>
        </footer>
    </div>

    <!-- Load real instance data -->
    ...
    <script>
            ...
            instanceData.forEach(instance => {{
                ...
                row.innerHTML = `
                    <td>
                        ...
                        <div class="api-name">${{instance.apiName || instance.name}}</div>
                        ${{instance.isNew ? '<span class="badge badge-new">NEW</span>' : ''}}
                    </td>
                    <td>
                        <div class="spec-value">${{instance.gpu}}</div>
                    </td>
                    <td><span class="spec-value">${{instance.gpuCount || 0}}</span><span class="spec-unit">×</span></td>
                    <td><span class="spec-value">${{instance.vcpu}}</span><span class="spec-unit">vCPUs</span></td>
                    <td><span class="spec-value">${{instance.memory.replace(' GB', '')}}</span><span class="spec-unit">GB</span></td>
                    <td><span class="spec-unit">${{instance.network}}</span></td>
                    <td><span class="spec-unit">${{instance.storage}}</span></td>
                    <td>${{isAvailable && pricing ? `
                        <div class="price-grid">
                            <div class="price-item">
                                <span class="price-label">On-Demand:</span>
                                <span class="price-value">$${{pricing.onDemand.toFixed(2)}}/hr</span>
                            </div>
                        </div>
                    ` : '<span style="color: #999;">Not Available</span>'}}</td>
                    <td>${{isAvailable ? '<span class="badge badge-region">Available</span>' : '<span style="color: #999;">Not Available</span>'}}</td>
                `;
        ...
        function changeRegion(region) {{
            ...
            document.getElementById('regionCode').textContent = region;
            document.getElementById('statsRegion').textContent = region;
            document.getElementById('currentRegionLabel').textContent = regionNames[region] || region;
            renderTable();
        }}
        ...
        filterInput.addEventListener('input', function() {{
            ...
            const rows = document.querySelectorAll('#instanceTableBody tr');
        ...
        function updateVisibleCount() {{
            ...
            document.getElementById('totalCount').textContent = instanceData.length;
        }}
...
def generate_page(family, family_config, instance_count):
    """生成单个系列的 HTML 页面"""

    # 确定面包屑导航
    if family.startswith('p'):
        breadcrumb = f"NVIDIA GPU / P Series / {family.upper()}"
    elif family.startswith('g'):
        breadcrumb = f"NVIDIA GPU / G Series / {family.upper()}"
    elif family.startswith('inf'):
        breadcrumb = f"AWS Custom / Inferentia / {family.upper()}"
    elif family.startswith('trn'):
        breadcrumb = f"AWS Custom / Trainium / {family.upper()}"
    else:
        breadcrumb = f"GPU Instances / {family.upper()}"

    html = HTML_TEMPLATE.format(
        title=family_config['title'],
        description=family_config['description'],
        instance_count=instance_count,
        year=family_config['year'],
        use_case=family_config['use_case'],
        family=family,
...
def main():
    data_dir = Path('/home/ubuntu/codes/aws-gpu/data')
    instances_dir = Path('/home/ubuntu/codes/aws-gpu/instances')
    instances_dir.mkdir(exist_ok=True)

    print("🚀 自动生成 GPU 实例详情页面\n")

    generated = []
    skipped = []

    # 遍历所有数据文件
    for data_file in sorted(data_dir.glob('*_instances.js')):
        family = data_file.stem.replace('_instances', '')

        # 跳过特殊文件
        if family in ['all_gpu', 'p5_family_all']:
            continue

        # 跳过已经手动创建的 p5
        if family == 'p5':
            print(f"⏭️  {family}: 已存在（手动创建），跳过")
    ...
    print(f"\n📊 生成统计:")
    print(f"   成功生成: {len(generated)} 个页面")
    print(f"   跳过: {len(skipped)} 个")

    if generated:
        print(f"\n✨ 生成的页面:")
        for family in generated:
            print(f"   - instances/{family}.html")
